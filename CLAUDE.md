...
- **PDF generation:** PDF.co API (`get_pdf.py`) — fetches the deployed preview URL and generates PDFs
...
Triggers on **push to any non-main branch** (and `workflow_dispatch`). Does NOT trigger on pull_request — this prevents double-runs when pushing to a branch with an open PR.

Flow:
1. Builds the Hugo site
2. Waits for Cloudflare Pages to deploy the preview (polls CF API, 30 retries × 10s)
3. Generates PDFs from the preview URL via PDF.co (`get_pdf.py --papersize Letter`)
4. Commits PDFs back to the branch (`static/*.pdf`)
...
Flow: same as above but uses the production URL. Regenerates PDFs from the live site after each merge to main.
...
RESUME_URL=https://resume.davidpacold.dev python get_pdf.py --papersize Letter
```
...
| `get_pdf.py` | PDF generation script using PDF.co API |
